...
{
$r['min_x'] = $userdata['x']-($x_size/2);
$r['max_x'] = $userdata['x']+($x_size/2);
$r['min_z'] = $userdata['z']-($z_size/2);
$r['max_z'] = $userdata['z']+($z_size/2);
} else {
...
$world_z_size = abs($world_z_max - $world_z_min);
$world_x_min = $r['min_x']-(0.05*$world_x_size);
$world_x_max = $r['max_x']+(0.05*$world_x_size);
$world_z_min = $r['min_z']-(0.05*$world_z_size);
$world_z_max = $r['max_z']+(0.05*$world_z_size);
$world_x_size = ($world_x_max - $world_x_min);
$world_z_size = ($world_x_max - $world_z_min);

$ratio = 1;
if(($x_size/$world_x_size) < ($z_size/$world_z_size))
{
    $ratio = $x_size/$world_x_size;
} else {
    $ratio = $z_size/$world_z_size;
}
